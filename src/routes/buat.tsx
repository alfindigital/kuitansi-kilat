              ...
              <Popover>
                ...
                <PopoverContent className="w-72 p-1.5" align="end">
                  <div className="max-h-80 overflow-auto space-y-2">
                    {presets.length > 0 && (
                      <div>
                        <div className="px-2 pt-1 pb-1 text-[10px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
                          Pilih preset
                        </div>
                        <div className="space-y-0.5">
                          {presets.map((p) => (
                            <button
                              key={p.id}
                              onClick={() => addPreset(p)}
                              className="tap w-full text-left px-2 py-2 text-sm rounded-lg hover:bg-accent flex justify-between gap-2"
                            >
                      ...
                      <div>
                        ...
                        <div className="space-y-0.5">
                          {savable.map((it, idx) => (
                            <button
                              key={`${it.name}-${idx}`}
                              onClick={() => savePresetMutation.mutate(it)}
                              className="tap w-full text-left px-2 py-2 text-sm rounded-lg hover:bg-accent flex items-center justify-between gap-2"
                            >
